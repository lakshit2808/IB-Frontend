...
const PositionHandling = () => {
  ...
  useEffect(() =>{
    axios.get('https://tradingviewsignal.herokuapp.com/v1/bot1/positionhandling')
    .then(res => {      
      setData(res.data);
    })
  },[])
  ...
  const deleteBotHandler = (index,ticker) => {
    const encoded = encodeURI(`https://tradingviewsignal.herokuapp.com/v1/bot1/positionhandling?tickerName=${ticker}`);
    let tempList = Data
    Data.splice(index, 1)
    setData(tempList)
  ...
  const addCloseHandler = async (tickerName) => {
    await axios.post('https://tradingviewsignal.herokuapp.com/v1/bot1/positionhandling/closeposition', {
